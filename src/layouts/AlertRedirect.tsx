...
import { isWebClip, isInAppBrowser } from "@/utils/browserDetection";
...
/**
 * Props for the AlertRedirect component
 */
interface AlertRedirectProps {
  /** Callback to control the visibility of the alert */
  setShowAlert: (show: boolean) => void;
  /** URL to the app download link */
  app_download_link: string;
  /** Whether this alert is shown during an event (prevents auto-play resume) */
  event?: boolean;
}

/**
 * AlertRedirect Component
 * 
 * Business Logic:
 * This component displays a modal alert that encourages users to download the mobile app.
 * It handles different scenarios:
 * 1. WebClip users (iOS standalone): Shows a message about App Store version with close button
 * 2. Regular browser users: Shows download prompt with option to continue in browser
 * 3. In-app browser detection: If user clicks download in WeChat/Alipay/Weibo/QQ, shows guide overlay
 * 
 * The component prevents video auto-play from resuming when closed during events.
 * 
 * SOLID Principles:
 * - Single Responsibility: Handles app download promotion UI and user flow
 * - Open/Closed: Extensible through props without modification
 * - Dependency Inversion: Depends on abstractions (utilities, Redux actions)
 */
const AlertRedirect: React.FC<AlertRedirectProps> = ({
  ...
  /**
   * Memoized check for WebClip mode (iOS standalone)
   * This determines if the user has installed the app as a web clip
   */
  const isWebClipMode = useMemo(() => isWebClip(), []);

  /**
   * Handles the download button click
   * Business Logic:
   * - If user is in an in-app browser (WeChat, Alipay, Weibo, QQ), show guide overlay
   *   because these browsers block external links and users need instructions
   * - Otherwise, open the download link in a new tab
   */
  const handleDownloadClick = useCallback(
    (e: React.MouseEvent<HTMLAnchorElement>) => {
      // Check if user is in a restricted in-app browser
      if (isInAppBrowser()) {
        e.preventDefault();
  ...
  /**
   * Handles the "Continue in Browser" button click
   * Business Logic:
   * - Closes the alert modal
   * - Resumes video playback unless this is an event (events may have special playback rules)
   */
  const handleBrowserContinue = useCallback(() => {
    setShowAlert(false);
    // Only resume playback if not in event mode
    // Events may have different playback requirements
    if (!event) {
      dispatch(setPlay(true));
    }
  }, [setShowAlert, event, dispatch]);

  /**
   * Handles the close button click (WebClip mode only)
   * Business Logic:
   * - Same as browser continue - closes alert and resumes playback if not in event
   */
   ...
   * Displays the app logo, description, and download button
   */
  const AppDownloadSection = () => (
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-3">
        <img src={logo} alt="笔盒APP Logo" className="w-[50px] h-[50px]" />
        <div>
  ...
  /**
   * Browser Option Section Component
   * Only shown for non-WebClip users, allowing them to continue in browser
   */
  const BrowserOptionSection = () => {
    if (isWebClipMode) return null;
    ...
    return (
      <div className="flex items-center justify-between">
        ...
        <div>
          <button className="alert-body-btn1" onClick={handleBrowserContinue}>
            继续
          </button>
        </div>
      </div>
  ...
  return (
    <>
      ...
      <div className="bg-black/80 w-screen flex flex-col gap-[20px] justify-center items-center fixed top-0 z-[999999] alert-height">
        ...
        >
          {/* Header Section */}
          {/* Business Logic: WebClip users see App Store message with close button,
              regular users see general download recommendation */}
          {isWebClipMode ? (
          ...
          <div className="flex flex-col mt-8 gap-5">
            ...
            {/* Business Logic: Shows guide image when user tries to download from
                restricted in-app browsers (WeChat, Alipay, Weibo, QQ) */}
            {showInAppBrowserAlert && <InAppBrowserAlert />}
            
            {/* Browser Continue Option */}
            {/* Business Logic: Only shown to non-WebClip users, allowing them
                to dismiss the alert and continue watching in browser */}
            <BrowserOptionSection />
          </div>
        </motion.div>
      </div>
    </>

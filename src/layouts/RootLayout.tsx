...
const RootLayout = ({ children }: any) => {
    ...
    const hideBar = useSelector(
        (state: RootState) => state.hideBarSlice.hideBar
    );
    const hideNew = useSelector(
        (state: RootState) => state.hideNewSlice.hideNew
    );
    ...
    useEffect(() => {
        const prefetchEventDetails = async () => {
            ...
            try {
                ...
                const eventDetails = await triggerGetEventDetails(
                    eventId
                ).unwrap();
        ...
        if (cachedEventDetails) {
            ...
            try {
                ...
                if (freshEventDetails.data?.event_start_time) {
                    dispatch(
                        setDuration(freshEventDetails.data.event_start_time)
                    );
                }
            } catch (error) {

...
const GossipReport = () => {
  ...
  const { data: reportsResponse, isLoading: isReportOptionsLoading } =
    useGetReportsQuery({});
  const [triggerReport, { isLoading }] = useStoreReportMutation();
  ...
  const handleReport = async (content: string) => {
    ...
    try {
      ...
      if (response?.data) {
        ...
        dispatch(
          gossipExternalApi.util.invalidateTags(
            categoryId
              ? [{ type: "gossipPosts", id: categoryId }]
              : [{ type: "gossipPosts" }]
          )
        );
        navigate(-1);
    ...
    <div className="text-white bg-[#16131C] min-h-screen">
      ...
      <div className="px-5 pt-2">
        {isReportOptionsLoading ? (
          ...
          Object.entries(reportGroups).map(([group, items], groupIndex) => (
            <div
              key={group}
              className={`mb-6 ${
                groupIndex !== Object.keys(reportGroups).length - 1

...
const TopRankCard = ({ data, rank }: { data: any; rank: any }) => {
  ...
  return (
    <div className="bg-red-900 w-[110px] h-[131px] border-[0px] relative flex flex-col justify-center items-center rounded-[8px] pt-5">
      <Avatar>
        <AvatarImage src={decryptedPhoto} alt="@shadcn" />
      </Avatar>
      <h1 className="text-[14px] font-semibold">{data?.nickname}</h1>
      <h1 className="text-[#AAA] text-[12px]">
        {data?.total >= 1000 ? formatToK(data?.total) : data?.total} followers
      </h1>
      {data?.id == me ? (
        <></>
      ) : (
        <RankBtn id={data?.id} followBack={data?.follows_back} />
      )}
